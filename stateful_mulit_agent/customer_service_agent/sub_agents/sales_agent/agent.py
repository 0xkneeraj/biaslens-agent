...
def purchase_course(tool_context: ToolContext):
    course_id = "ai_marketing_platform"
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    # Get current purchased courses
    current_puchased_course = tool_context.state.get("purchased_history", [])
    # Check if user already owns the course
    course_ids  = [
        course['id'] for course in current_puchased_course if isinstance(course, dict)
    ]
    if course_id in course_ids:
        return {"status": "error", "message": "You already own this course!"}
    # create new list of course 
    new_purchased_courses = []
    for course in current_puchased_course:
    ...
    tool_context.state["purchased_history"] = new_purchased_courses
...
sales_agent = Agent(
    ...
    <purchase_info>
    Purchased Courses: {purchased_history}
    </purchase_info>
